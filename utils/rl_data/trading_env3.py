...
class TradingEnv(gym.Env):
    ...
    def __init__(
        ...
        max_slippage_pips: float = 5.0, # Random slippage up to $5
    ...
    def step(self, action: int):
        ...
        if act_type == "HOLD":
            ...
            if self.position != 0:
                exit_price = self._apply_slippage(current_price, is_entry=False)
    ...
    def _open_position(self, current_price, direction, sl_ratio, tp_ratio):
        entry_price = self._apply_slippage(current_price, is_entry=True)
    ...
    def _apply_slippage(self, price, is_entry=True):
        if self.max_slippage_pips <= 0: return price
        
        slip = np.random.uniform(0, self.max_slippage_pips) * self.pip_value
